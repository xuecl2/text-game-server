...
export function login(req, ws) {
    ...
    if(!name || !passwd) return utils.getFailureRsp(moduleCode + '001', '用户名或密码不能为空！')
    if(!armies.get(armyNames.get(name)) || armies.get(armyNames.get(name)).passwd !== passwd) return utils.getFailureRsp(moduleCode + '002', '用户名或密码错误！')
